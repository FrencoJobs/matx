...
function dot(x,y){
  ...
  if(x.length == y.length){
    ...
    return res.splice(0,res.length).reduce(function(x,y){return x+y;});
  }
  return false;
}
...
function create(arr,size){
  if(nonZero(arr,size) && isObject(arr) && isObject(size) && arr.length == size.reduce(function(x,y){return x*y;})){
    let mat = new Array();
    for(let i = 0;i<size[0];i++){
      mat.push(arr.splice(0,size[1]));
    }
    return {output:mat.splice(0,mat.length),
            size:size};
  }
  return false;
}
...
function cvec(arr){
  return {output:arr.map(function(x){return [x]}),size:[arr.length,1]}
}
